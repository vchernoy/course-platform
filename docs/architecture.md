...
- **Auth:** Clerk (middleware) plus **[`config/admins.yaml`](../config/admins.yaml)** — preferred **`assignments`** with **`scope.type`** **`platform`** (all offerings and all sites) \| **`offering`** \| **`site`**, plus **backward-compatible** **`role`** + **`offerings`** + optional **`sites`** (omit **`sites`** → no site admin UI for that legacy row). Roles do not yet differentiate behavior beyond **`canAdminAccess*`** helpers. **`"*"`** in legacy `offerings` or `sites` means all of that dimension. Route handlers and server actions use [`lib/admin-auth.ts`](../lib/admin-auth.ts) (`canAdminAccessOffering`, `canAdminAccessSite`, etc.). See [Admin access model](./admin-authoring.md#admin-access-model); **page** / **lesson** scopes are [future-only](./admin-authoring.md#future-planned-scoped-role-assignments).
